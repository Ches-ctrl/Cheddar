class JobsController < ApplicationController
  ...
  def index
    ...
    @jobs = if params[:query].present?
              Job.with_location.search_job(params[:query]).includes(:company).paginate(page: params[:page], per_page: 10)
            else
              Job.with_location.includes(:company).paginate(page: params[:page], per_page: 10)
            end
  end

  def show
    @job = Job.find(params[:id]).includes(:company) # we can call @job.company.company_name or any other company attributes
    @company = @job.company # No need for this line with the line above.
    @saved_job = SavedJob.new # why do we have this here?
    @job_description = sanitize @job.job_description # why this?
  end
