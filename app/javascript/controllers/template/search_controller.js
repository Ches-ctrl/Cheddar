...
export default class extends Controller {
  static targets = ["searchDisplay", "searchInput"];

  connect() {
    this.searchDisplayTarget.addEventListener("click", (event) => {
    ...
    return () => {
      this.searchDisplayTarget.removeEventListener("click", (event) => {
        this.handleClick(event);
      });
  ...
  handleKeyDown(event) {
    // Call isShortcut after it's defined
    if (this.isShortcut(event, "k", "metaCtrl")) {
      event.preventDefault();
      this.showSearch();
    } else if (event.key === "Escape") {
      event.preventDefault();
  ...
  showSearch() {
    this.searchDisplayTarget.classList.remove("hidden");
    this.searchInputTarget.focus();
    this.searchInputTarget.value = "";
  };

  hideSearch() {
    this.searchDisplayTarget.classList.add("hidden");
    this.searchInputTarget.value = "";
  };
}