  ...
  def fill_out_form(url, fields, job_application_id)
    ...
    begin
      ...
      fields.each do |field|
        field[1]
        if field[0] == 'resume'
          file_path = Rails.root.join('tmp', "Resume - #{@user.first_name} #{@user.last_name} - - #{@job.job_title} - #{@job.company.name}.pdf")
          FileUtils.rm_f(file_path)
        elsif field[0] == 'cover_letter_'
          file_path = Rails.root.join('tmp',
                                      "Cover Letter - #{@job.job_title} - #{@job.company.name} - #{@user.first_name} #{@user.last_name}.docx")
          FileUtils.rm_f(file_path)
        end
      end
